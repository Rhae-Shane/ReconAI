...
export function JournalPeriods() {
  ...
  const postRun = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch("/api/close/runs/run_today/post", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ periodId: open?.id }),
      });
      const body = await res.json();
      if (!res.ok) throw new Error(body.error ?? body.reason ?? "Post failed");
    } catch (err) {
      console.error(err);
    } finally {
      setPosting(false);
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-2">
          ...
          <Button size="sm" disabled={posting || busy} onClick={() => void postRun()}>
            {posting ? "Posting…" : "Post latest close to GL"}
          </Button>
        </CardContent>
      </Card>
