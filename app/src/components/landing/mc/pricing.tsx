...
const PLANS: Plan[] = [
  {
    ...
    price: 0,
    memories: 300,
    features: ["300 events", "Limited retrieval", "Unlimited projects", "Razorpay test mode", "Community support"],
  },
  {
    id: "hobby",
    name: "Hobby",
    ...
    memories: 2000,
    popular: true,
    features: ["2,000 events", "Unlimited retrieval", "Unlimited projects", "Razorpay test mode", "Priority support"],
  },
  {
    id: "pro",
    name: "Pro",
      ...
      <div className="group relative flex h-full flex-col">
        ...
        <div className="relative">
          ...
          <svg
            ...
            className={`relative z-10 h-auto w-full drop-shadow-[0_8px_24px_rgba(232,97,60,0.3)] sm:hidden`}
          >
            <defs>
              <linearGradient
              ...
              >
                <stop offset="0%" stopColor="#E8613C" stopOpacity={isPopular ? "0.9" : "0.75"} />
                <stop offset="100%" stopColor="#C94E2E" stopOpacity={isPopular ? "0.8" : "0.6"} />
              </linearGradient>
              <radialGradient
            ...
            <path
              ...
              fill="none"
            />
            <path d="M0 8 V100" stroke={`url(#folderEdgeLeftSm-${plan.id})`} strokeWidth="1.5" fill="none" />
          </svg>
          ...
          <svg
            ...
            fill="none"
            preserveAspectRatio="none"
            className={`relative z-10 hidden h-auto w-full drop-shadow-[0_8px_24px_rgba(232,97,60,0.3)] sm:block`}
          >
            <defs>
              <linearGradient
            ...
            />
            {/* Left edge highlight */}
            <path d="M0 8 V150" stroke={`url(#folderEdgeLeft-${plan.id})`} strokeWidth="1.5" fill="none" />
          </svg>

          {/* Folder content: plan name, price, CTA */}
          <div className="absolute inset-0 z-20 flex flex-col justify-center px-5 pb-4 md:pt-3">
            {/* Plan name + badge */}
            <div className="mb-2 flex items-center justify-between">
              <h3 className="font-bold font-display text-2xl text-white tracking-tight sm:text-3xl">{plan.name}</h3>
              {isPopular && (
                <span className="rounded-md bg-white/15 px-2 py-0.5 font-semibold text-[10px] text-white/90 uppercase tracking-wide backdrop-blur-sm">
                  Popular
                </span>
              )}
            </div>
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="font-display font-extrabold text-4xl text-white tracking-tighter sm:text-5xl">
                ${plan.price}
              </span>
              <span className="font-medium text-sm text-white/60 sm:text-base">/month</span>
            </div>

            {/* Memories included */}
            <p className="mt-1 text-white/50 text-xs">{plan.memories.toLocaleString()} events included</p>
          </div>
        </div>

        {/* ── Bottom: Features Card with corner brackets ── */}
        <div
          className={`relative -mt-4 flex flex-1 flex-col overflow-hidden rounded-xl ${
            isPopular ? "border border-accent/25 bg-accent/[0.06]" : "border border-white/10 bg-border-hover/30"
          }`}
        >
          {/* Inner card with L-shaped corner brackets */}
          <div
            className={`relative m-2 flex flex-1 flex-col rounded-xl ${
              isPopular ? "border border-accent/15 bg-background/90" : "border border-white/10 bg-background/80"
            }`}
          >
            {/* Top Left */}
            <GrTopCorner
              className={`absolute top-2 left-2 h-4 w-4 ${isPopular ? "text-accent/50" : "text-foreground-muted"}`}
            />
            {/* Top Right */}
            <GrTopCorner
              className={`absolute top-2 right-2 h-4 w-4 rotate-90 ${isPopular ? "text-accent/50" : "text-foreground-muted"}`}
            />
            {/* Bottom Right */}
            <GrTopCorner
              className={`absolute right-2 bottom-2 h-4 w-4 rotate-180 ${isPopular ? "text-accent/50" : "text-foreground-muted"}`}
            />
            {/* Bottom Left */}
            <GrTopCorner
              className={`absolute bottom-2 left-2 h-4 w-4 -rotate-90 ${isPopular ? "text-accent/50" : "text-foreground-muted"}`}
            />
            ...
            <div className="relative flex flex-1 flex-col p-5 pt-6">
              ...
              {/* CTA Button */}
              {isPopular ? (
                <a href="/auth/v1/login" className="group/btn relative block w-full">
                  <div
                    className="absolute -top-px -left-px h-8 w-16 rounded-xl opacity-80 blur-[0.5px] transition-opacity group-hover/btn:opacity-100"
                    style={{
                      background:
                  ...
                  />
                  <div
                    className="relative flex items-center justify-center gap-2 overflow-hidden rounded-xl px-5 py-3 transition-all group-hover/btn:scale-[1.02]"
                    style={{
                  ...
                  >
                    ...
                    />
                    <span className="relative z-10 font-display font-semibold text-sm text-white">
                      Start with {plan.name}
                    </span>
                    <ArrowRight className="relative z-10 h-4 w-4 text-white transition-transform group-hover/btn:translate-x-1" />
                  </div>
                </a>
              ) : (
                <a href="/auth/v1/login" className="group/btn relative block w-full">
                  <div
                    className="absolute -top-px -left-px h-7 w-14 rounded-xl blur-[0.5px]"
                    style={{
                      background:
                  ...
                  <div className="relative flex items-center justify-center gap-2 rounded-xl border border-white/[0.08] bg-surface/60 px-5 py-3 backdrop-blur-sm transition-all group-hover/btn:border-white/15 group-hover/btn:bg-surface/80">
                    <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-white/[0.04] via-transparent to-transparent" />
                    <span className="relative z-10 font-display font-semibold text-foreground text-sm">
                      {plan.id === "free" ? "Start for Free" : `Start with ${plan.name}`}
                    </span>
                    <ArrowRight className="relative z-10 h-4 w-4 text-foreground-muted transition-transform group-hover/btn:translate-x-1" />
                  </div>
                </a>
              )}
            </div>
...
export function Pricing() {
  return (
    <section className="relative overflow-hidden px-4 py-20 sm:px-6 md:py-28">
      ...
      <div className="pointer-events-none absolute top-0 left-0 h-75 w-75 sm:h-100 sm:w-100">
      ...
      <div className="pointer-events-none absolute top-0 right-0 h-75 w-75 sm:h-100 sm:w-100">
        <div className="relative h-full w-full">
          {rightDots.map((dot, i) => (
            <div
              key={`right-${i}`}
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mb-12 text-center sm:mb-14">
          ...
          <div className="mb-6 flex justify-center">
            <div className="group relative">
              ...
              <div className="relative inline-flex items-center rounded-full bg-surface/95 px-4 py-2 backdrop-blur-sm">
                ...
                <span className="relative z-10 font-medium text-foreground text-xs sm:text-sm">Pricing</span>
              </div>
            </div>
          </div>
          ...
          <p className="mx-auto max-w-2xl text-base text-foreground-muted sm:text-lg">
            Start free, scale as your volume grows. No hidden fees, no surprises.
          </p>
        </div>

        {/* Plans Grid */}
        <div className="mx-auto grid max-w-4xl grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3">
