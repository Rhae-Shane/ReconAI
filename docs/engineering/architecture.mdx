---
title: Architecture
description: System shape, FinanceCore pipeline, ops modes, UI map, and measured baseline for ReconAI.
---

Settlement reconciliation controller: match gateway, bank UTR, ERP, and GST lines; settle; forecast; tax-map; close with an **honest** exception list.
...
Currency is integer **paise** (signed). Types in `harness/src/core/types.ts` are authoritative; Prisma mirrors them for optional Postgres persistence.
...
Reason codes include `NO_KEY`, `AMOUNT_MISMATCH`, `PARTIAL_FLAP`, `DATE_SKEW`, `LOW_CONFIDENCE`, `DUPLICATE`.
