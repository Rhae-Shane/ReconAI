---
title: "Deployment"
description: "Deploy ReconAI locally or on Vercel, with optional Redis workers and production checklist."
---

Deploy the Next.js dashboard (`next-shadcn-admin-dashboard/`) and optionally a durable BullMQ worker. The deterministic harness in `harness/` has **no runtime deps** and is linked into the app as `controller-harness` (`file:../harness`).
...
3. Enable **Include source files outside of the Root Directory in the Build Step** so `file:../harness` resolves.
