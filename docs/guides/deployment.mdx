...
Deploy the Next.js dashboard (`app/`) and optionally a durable BullMQ worker. The deterministic harness in `harness/` has **no runtime deps** and is linked into the app as `controller-harness` (`file:../harness`).
