...
/**
 * POST /api/control/approvals
 *
 * Interact with the approval workflow (submit/review/approve/post/reject). The
 * `requestedBy` actor can never approve (or reject) their own request — that is
 * enforced by the harness `ApproverPolicy` (segregation of duties), and
 * high-value adjustments require dual approval. The policy instance is a fresh
 * per-request stateless evaluator: it enforces lifecycle rules and cardinality.
 */
export async function POST(request: Request) {
  // Approval actions advance financially-significant state (submit/review/approve/post/reject) —
  // restricted to the owner role (admin-only).
  ...
  try {
    if (action === "submit") {
      // Fresh DRAFT request from the proposed adjustment, then advance to SUBMITTED.
      ...
      // Reconstruct the immutable request record for review/approve/post/reject.
  ...
  await recordAudit({
    ...
    role: verdict.role,
    action: `approvals:${String(action)}`,
    target: requestId,
  });
